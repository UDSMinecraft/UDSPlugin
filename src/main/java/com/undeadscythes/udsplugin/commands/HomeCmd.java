...
public class HomeCmd extends CommandHandler {
    @Override
    public void playerExecute() throws NoPlayerFoundException, PlayerNotOnlineException {
        Region home;
        ...
        if(args.length == 0) {
            ...
            if(subCmdEquals("make")) {
                if(canAfford(Config.HOME_COST) && hasNoHome()) {
                    ...
                    home = new Region(player.getName() + "home", min, max, player.getLocation(), player.getOfflineMember(), "", RegionType.HOME);
                    if(noOverlaps(home)) {
                ...
                for(Region otherHome : RegionUtils.getRegions(RegionType.HOME)) {
                    if(otherHome.hasMember(player.getOfflineMember())) {
                    ...
                    if((home = RegionUtils.getRegion(RegionType.HOME, player.getName() + "home")) != null) {
                        for(OfflineMember member : home.getMembers()) {
            ...
            if(subCmdEquals("expand")) {
                ...
                if((target = matchPlayer(args[1])) != null && (home = getHome()) != null) {
                    ...
                    try {
                        MemberUtils.getOnlineMember(target).sendNormal("You have been added as " + player.getNick() + "'s room mate.");
                ...
                if((target = matchPlayer(args[1])) != null && (home = getHome()) != null && isRoomie(target, home)) {
                    ...
                    try {
                        MemberUtils.getOnlineMember(target).sendNormal("You are no longer " + player.getNick() + "'s room mate.");
                    } catch(PlayerNotOnlineException ex) {}
                }
            } else {
                subCmdHelp();
