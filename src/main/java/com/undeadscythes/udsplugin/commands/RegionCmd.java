...
public class RegionCmd extends CommandHandler {
    @Override
    public void playerExecute() throws NoPlayerFoundException, NoEnumFoundException {
        if(args.length == 1) {
    ...
    private void contract() {
        ...
        Region region;
        int distance;
        if((region = getRegion(args[1])) != null && (direction = getCardinalDirection(args[3])) != null && (distance = getInteger(args[2])) > -1) {
    ...
    private void addMember() throws NoPlayerFoundException {
        final Region region = getRegion(args[1]);
        final OfflineMember target = matchPlayer(args[2]);
    ...
    private void delMember() throws NoPlayerFoundException {
        final Region region = getRegion(args[1]);
        final OfflineMember target = matchPlayer(args[2]);
        if(region != null && target != null) {
    ...
    private void owner() throws NoPlayerFoundException {
        final Region region = getRegion(args[1]);
        final OfflineMember target = matchPlayer(args[2]);
        if(region != null && target != null) {
    ...
    private void changeRank() throws NoEnumFoundException {
        final Region region = getRegion(args[1]);
        final MemberRank rank = getRank(args[2]);
